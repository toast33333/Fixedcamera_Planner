...
public:
    explicit PNGInterceptor(ros::NodeHandle& nh) {
        ...
        if (msg_mode == 1) {
            ...
            ROS_INFO("[LOSBboxVz] Subscribed to %s (detection_msgs/Detection)", topic_name.c_str());
        } else {
            sub_det_kcf_ = nh.subscribe<kcf_msgs::Bbox>(
                topic_name, 1, &PNGInterceptor::detectionCbKcf, this);
            ROS_INFO("[LOSBboxVz] Subscribed to %s (kcf_msgs/Bbox)", topic_name.c_str());
        }
        ...
        ROS_INFO("[LOSBboxVz] Started (forward LOS/yaw tracking + z from bbox y).");
        ROS_INFO("[LOSBboxVz] Yaw is enabled after horizontal speed reaches %.3f m/s.",
                 yaw_enable_speed);
    }
...
private:
    void updateYawEnableState() {
        if (yaw_control_enabled_ || !have_vel_) {
            if (!have_vel_) {
                ROS_WARN_THROTTLE(1.0,
                                  "[LOSBboxVz] Waiting for velocity on %s, yaw control remains disabled.",
        ...
        if (current_speed_ >= yaw_enable_speed) {
            ...
            ROS_INFO("[LOSBboxVz] Horizontal speed %.3f m/s reached threshold %.3f m/s, yaw control enabled.",
                     current_speed_, yaw_enable_speed);
        } else {
            ROS_INFO_THROTTLE(1.0,
                              "[LOSBboxVz] Accelerating: horizontal speed %.3f m/s, yaw enable threshold %.3f m/s.",
                              current_speed_, yaw_enable_speed);
        }
    }
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "rp_los_bbox_vz_node");
    ros::NodeHandle nh("~");
