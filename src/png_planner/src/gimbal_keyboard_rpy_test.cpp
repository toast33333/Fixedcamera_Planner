    ...
    double yaw_deg)
{
    ...
    const uint8_t angle_lock_mode = static_cast<uint8_t>((0u << 6) | (1u << 4) | (0u << 3));

    pkt[4] = angle_lock_mode;
    putInt16LE(pkt, 5, static_cast<int16_t>(std::lround(roll_deg * 100.0)));

    pkt[7] = angle_lock_mode;
    putInt16LE(pkt, 8, static_cast<int16_t>(std::lround(pitch_deg * 100.0)));

    pkt[10] = angle_lock_mode;
...
void printRxBrief(const std::vector<uint8_t>& f, double roll_cmd, double pitch_cmd, double yaw_cmd)
{
    if (f.size() != 26) {
        return;
    }

    const uint8_t fw_ver = f[2];
    const uint8_t hw_err = f[3];

    const uint8_t flag = f[4];
    const uint8_t inv_flag = flag & 0x01;
    const uint8_t gbc_stat = (flag >> 1) & 0x07;
    const uint8_t tca_flag = (flag >> 4) & 0x01;

    const uint8_t cmd = f[5];
    const uint8_t cmd_stat = cmd & 0x07;
    const uint8_t cmd_value = (cmd >> 3) & 0x1F;

    const double cam_rate_pitch = getInt16LE(&f[6]) * 0.1;
    const double cam_rate_roll = getInt16LE(&f[8]) * 0.1;
    const double cam_rate_yaw = getInt16LE(&f[10]) * 0.1;

    const double cam_angle_roll = getInt16LE(&f[12]) * 0.01;
    const double cam_angle_pitch = getInt16LE(&f[14]) * 0.01;
    const double cam_angle_yaw = getInt16LE(&f[16]) * 0.01;

    const double mang_pitch = getInt16LE(&f[18]) * 0.01;
    const double mang_roll = getInt16LE(&f[20]) * 0.01;
    const double mang_yaw = getInt16LE(&f[22]) * 0.01;
    ...
    std::cout << "\r"
              ...
              << std::fixed
              << " | cmd[r,p,y]=[" << std::setprecision(2)
              << roll_cmd << "," << pitch_cmd << "," << yaw_cmd << "] deg"
              << " | cam_rate[p,r,y]=[" << std::setprecision(1)
              << cam_rate_pitch << "," << cam_rate_roll << "," << cam_rate_yaw << "] deg/s"
              << " | cam_att[r,p,y]=[" << std::setprecision(2)
              << cam_angle_roll << "," << cam_angle_pitch << "," << cam_angle_yaw << "] deg"
              << " | mang[p,r,y]=["
              << mang_pitch << "," << mang_roll << "," << mang_yaw << "] deg     "
              << std::flush;
}
...
int main(int argc, char** argv)
{
    ...
    const double yaw_limit_deg = 145.0;
    ...
    printHelp(dev, step_deg, roll_limit_deg, pitch_limit_deg, yaw_limit_deg);
    ...
    while (g_running) {
        ...
        if (readKeyNonBlocking(key, 0)) {
            if (key == 'q' || key == 'Q') {
                ...
                roll_deg = clampValue(roll_deg + step_deg, -roll_limit_deg, roll_limit_deg);
                std::cout << "\nSet roll  = " << roll_deg << " deg\n";
            } else if (key == 'd' || key == 'D') {
                roll_deg = clampValue(roll_deg - step_deg, -roll_limit_deg, roll_limit_deg);
                std::cout << "\nSet roll  = " << roll_deg << " deg\n";
            } else if (key == 'w' || key == 'W') {
                pitch_deg = clampValue(pitch_deg + step_deg, -pitch_limit_deg, pitch_limit_deg);
                std::cout << "\nSet pitch = " << pitch_deg << " deg\n";
            } else if (key == 's' || key == 'S') {
                pitch_deg = clampValue(pitch_deg - step_deg, -pitch_limit_deg, pitch_limit_deg);
                std::cout << "\nSet pitch = " << pitch_deg << " deg\n";
            } else if (key == 'l' || key == 'L') {
                yaw_deg = clampValue(yaw_deg + step_deg, -yaw_limit_deg, yaw_limit_deg);
                std::cout << "\nSet yaw   = " << yaw_deg << " deg\n";
            } else if (key == 'j' || key == 'J') {
                yaw_deg = clampValue(yaw_deg - step_deg, -yaw_limit_deg, yaw_limit_deg);
                std::cout << "\nSet yaw   = " << yaw_deg << " deg\n";
        ...
        auto tx = buildTxPacket(!manual_cmd_sent, trig, roll_deg, pitch_deg, yaw_deg);
        ...
        if (!latest_rx.empty() && loop_count % 5 == 0) {
            printRxBrief(latest_rx, roll_deg, pitch_deg, yaw_deg);
        }
